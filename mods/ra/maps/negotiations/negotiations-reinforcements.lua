...
ReinforceChronoTanks = function(locations)
	...
	Utils.Do(locations, function(location)
		local proxy = SpawnMiscActor("powerproxy.chronoshift", GoodGuy)
...
PrepareSouthChinook = function()
	...
	Trigger.OnEnteredProximityTrigger(SouthChinookProximity.CenterPosition, WDist.FromCells(8), function(actor, id)
		if not (EscortFinished and actor.Type == TanyaType) then
			return
		end
...
ReinforceHardDogs = function()
	local startDog = CreateHardDog(WestRoadEntry.Location, Angle.South)
...
CreateHardDog = function(location, facing)
	return Actor.Create("dog.long", true, { Owner = USSR, Location = location, Facing = facing or Angle.South })
end
