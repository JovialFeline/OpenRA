...
PrepareBadGuy = function()
	...
	SetBotStructures(
		{ type = "powr", exists = false, footprint = "2x3", location = CPos.New(40, 51) },
		{ type = "proc", exists = false, footprint = "3x4", location = CPos.New(33, 52) },
		{ type = "barr", exists = false, footprint = "2x3", location = CPos.New(40, 55), onBuilt = BuildInfantry, rebuildSkipped = true },
		{ type = "powr", exists = false, footprint = "2x3", location = CPos.New(30, 50) },
		{ type = "ftur", exists = false, footprint = "1x1", location = CPos.New(39, 58) },
		{ type = "ftur", exists = false, footprint = "1x1", location = CPos.New(43, 57) },
		{ type = "powr", exists = false, footprint = "2x3", location = CPos.New(30, 53) },
		{ type = "weap", exists = false, footprint = "3x3", location = CPos.New(33, 56), onBuilt = BuildVehicles, rebuildSkipped = true },
		-- Tesla coil is originally built before factory.
		-- For convention, coil is swapped (and service depot added).
		{ type = "tsla", exists = false, footprint = "1x1", location = CPos.New(37, 57) },
		{ type = "fix", exists = false, footprint = "3x3", location = CPos.New(30, 56) }
	)

	Trigger.OnEnteredFootprint( { BuilderRally.Location }, function(actor, id)
		if not (actor.Type == "fact") then
			return
		end
...
ActivateBot = function()
	CheckStructures(BadGuy, DateTime.Seconds(10))
...
ScheduleStructure = function(player, structure, buildTime, interval)
	Trigger.AfterDelay(buildTime, function()
		...
		if not success and blocked then
			CheckStructures(player, interval, interval / 5)
			return
		end
...
BuildStructure = function(player, structure)
	...
	local blocked = IsStructureAreaBlocked(player, structure.position, structure.footprint)
	if blocked then
...
StructureFootprints =
{
	...
	["2x1"] = WVec.New(2 * 1024, 1 * 1024, 0),
	["2x3"] = WVec.New(2 * 1024, 3 * 1024, 0),
	["3x3"] = WVec.New(3 * 1024, 3 * 1024, 0),
	["3x4"] = WVec.New(3 * 1024, 4 * 1024, 0),
}

IsStructureAreaBlocked = function(player, position, dimensions)
	local foot = StructureFootprints[dimensions]
...
ScatterBlockers = function(player, actors)
	Utils.Do(actors, function(actor)
		if not (actor.IsIdle and actor.Owner == player) then
			return
		end
		actor.Scatter()
	end)
